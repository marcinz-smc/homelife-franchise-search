...
export type RecoTab = "overview" | "approach" | "signals" | "registry";

export function RecoDetail({
  brokerage,
  onSelectLocation,
  idPrefix = "drawer",
}: {
  ...
  const [tab, setTab] = useState<RecoTab>("overview");
  ...
  const tabs = useMemo(() => {
    const items: { id: RecoTab; label: string }[] = [{ id: "overview", label: "Overview" }];
    if (lead) {
